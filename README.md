...
> **"Can my PC run this AI model?"** — like *Can You Run It*, but for local LLMs.
...
| `--min-params <n>` | Only show models ≥ nB parameters |
| `--max-params <n>` | Only show models ≤ nB parameters |
...
python checker.py --min-params 7 --max-params 13 # 7B–13B range
...
```
Score = (RAM ratio × 0.40) + (VRAM ratio × 0.40) + (CPU score × 0.20)
```

On Apple Silicon, the formula shifts to `(RAM ratio × 0.80) + (CPU score × 0.20)` since unified memory means RAM and VRAM are the same pool.

| Score | Status |
|---|---|
| ≥ 1.4 | ✅ Smooth |
| 1.0 – 1.4 | ⚡ Acceptable |
| 0.7 – 1.0 | ⚠️ Lag Likely |
| < 0.6 RAM, or no GPU | 🐢 CPU Only |
| < 0.6 RAM | ❌ Won't Run |
...
The tool ships with `models_builtin.json` — a committed database of 35+ popular models that works offline with no setup.
...
1. `models_db.json` — if it exists from a previous `--update` run
2. `models_builtin.json` — the committed fallback, always present

---

## Adding Custom Models

To add your own models, edit `models_builtin.json` — no Python knowledge needed:
...
```
llm-compat-checker/
├── checker.py              Main script — run this
├── models_builtin.json     Committed model database (edit to add custom models)
├── models_db.json          Auto-generated by --update (gitignored)
├── requirements.txt
├── .gitignore
└── scripts/
    └── scrape_hf_models.py Standalone HuggingFace scraper with extra options
```
...
| Qwen2.5 (Alibaba) | 1.5B → 72B, Coder, Vision |
...
- Ollama integration — list and check locally installed models
- LM Studio model detection
- Disk space check — estimate GGUF download size
